...
public class TestPhysicsRayCast extends SimpleApplication {
    ...
    public void simpleUpdate(float tpf) {
        List<PhysicsRayTestResult> rayTest = bulletAppState.getPhysicsSpace().rayTest(cam.getCamera().getLocation(), cam.getCamera().getLocation().add(cam.getCamera().getDirection()));
        if (rayTest.size() > 0) {
